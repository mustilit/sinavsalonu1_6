...
export default function TestDetail() {
  ...
  const { data: purchases = [] } = useQuery({
    queryKey: ["purchases", user?.email, testId],
    queryFn: () => base44.entities.Purchase.filter({ user_email: user.email, test_package_id: testId }),
    enabled: !!user && !!testId,
  });
  ...
  const { data: allTestResults = [] } = useQuery({
    queryKey: ["allTestResults", user?.email, testId],
  ...
  const { data: allTestProgress = [] } = useQuery({
    queryKey: ["allTestProgress", user?.email, testId],
  ...
  const { data: existingTestReview } = useQuery({
    queryKey: ["testReview", testId, user?.email],
    queryFn: async () => {
  ...
  const { data: follows = [] } = useQuery({
    queryKey: ["follows", user?.email, test?.educator_email],
    queryFn: () => base44.entities.Follow.filter({ follower_email: user.email, educator_email: test.educator_email }),
    enabled: !!user && !!test?.educator_email,
  });

  const isFollowing = follows.length > 0;

  const { data: educatorUser } = useQuery({
    queryKey: ["educatorUser", test?.educator_email],
    queryFn: async () => {
      const users = await base44.entities.User.filter({ email: test.educator_email });
      return users[0] || null;
    },
  ...
  const purchaseMutation = useMutation({
    mutationFn: async () => {
      await base44.entities.Purchase.create({
        ...
        status: "completed",
        test_package_snapshot: {
          id: test.id,
          title: test.title,
          description: test.description,
          cover_image: test.cover_image,
          exam_type_id: test.exam_type_id,
          exam_type_name: test.exam_type_name,
          difficulty: test.difficulty,
          duration_minutes: test.duration_minutes,
          is_timed: test.is_timed,
          has_solutions: test.has_solutions
        },
        tests_snapshot: tests,
        questions_snapshot: questions
      });
      // Update total sales
      await base44.entities.TestPackage.update(test.id, {
        total_sales: (test.total_sales || 0) + 1
      });
    },
    onSuccess: () => {
  ...
  const handleSubmitTestReview = async () => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ["testReview", testId, user?.email] });
              ...
              >
                ...
                <div>
                  <p className="font-semibold text-slate-900 hover:text-indigo-600 transition-colors">
                    {test.educator_name || educatorUser?.full_name || "Eğitici"}
                  </p>
                </div>
              </Link>
              {user && (
